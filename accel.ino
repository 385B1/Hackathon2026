...
void loop() {
  ...
  if (!sleep_) {
    ...
    int x2 = analogRead(JOYX);
    int y2 = analogRead(JOYY);

    //range_attune(&x2, &y2);

    x_map = map(x2, 0, 4095, 0, 6);
    y_map = map(y2, 0, 4095, 0, 6);

    determine_action_end(x2, y2);
  ...
  if (millis() - millis_var > 29297) {
    sleep_ = true;
    int8_t val = (int8_t)(((millis() / 20) % 255) - 128);
    if (val < 0) {
      val = -val;
    }
    leds[0] = (((uint8_t)val) << 16) | (((uint8_t)val) << 8);
    FastLED.show();
  }
